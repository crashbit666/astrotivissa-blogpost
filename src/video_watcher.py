...
def is_review_video(video):
    keywords = ['review', 'astrofotografia', 'review astrofotografia']
...
def watch_new_videos():
    ...
    for video in new_review_videos:
        ...
        process_new_video(video_id, video['snippet']['title'])
        processed_videos.append(video_id)

    save_processed_videos(processed_videos_path, processed_videos)


def process_new_video(video_id, blog_title):
    ...
    # Obtain the transcript for the video
    ...
    # Create blog post in the "reviews" category
    category_id = 283  # ID of the "reviews" category in the blog
    post_response = create_blog_post(blog_title, summary, category_id)
